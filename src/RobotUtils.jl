    ...
    if vertlbl[1] == sym
      ...
      if from <= val && val <= to
        if length( getOutNeighbors(fg, id[2] ) ) >= minnei
          ...
          X=[X; vec(getVal(fg,id[2], api=IncrementalInference.localapi)[1,:]) ]
          Y=[Y; vec(getVal(fg,id[2], api=IncrementalInference.localapi)[2,:]) ]
...
function get2DSampleMeans(fg::FactorGraph, sym; from::Int64=0, to::Int64=9999999999, minnei::Int64=0)
  ...
  for id in allIDs
    X=[X;Base.mean( vec( getVal(fg,string(sym,id), api=IncrementalInference.localapi)[1,:] ) )]
    Y=[Y;Base.mean( vec( getVal(fg, string(sym,id), api=IncrementalInference.localapi)[2,:] ) )]
    if sym == 'x'
      Th=[Th;Base.mean( vec( getVal(fg, string(sym,id), api=IncrementalInference.localapi)[3,:] ) )]
    end
