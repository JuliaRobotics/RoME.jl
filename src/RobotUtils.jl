...
function get2DSamples(fg::G;
                      from::Int=0, to::Int=(2^Sys.WORD_SIZE-1),
                      minnei::Int=0,
...
function listVariablesLabelsWithinRange(fg::AbstractDFG,
  ...
  for id in saids
    ...
    if occursin(regexKey, string(id)) && (from != 0 || to != (2^Sys.WORD_SIZE-1))
      vertlbl = string(id)
...
@deprecate getVariablesLabelsWithinRange(fg::AbstractDFG,regexKey::Regex=r"x";from::Int=0, to::Int=(2^Sys.WORD_SIZE-1),minnei::Int=0) listVariablesLabelsWithinRange(fg,regexKey,from=from,to=to,minnei=minnei)
...
function get2DPoseSamples(fg::G; from::Int=0, to::Int=(2^Sys.WORD_SIZE-1), regexKey=r"x") where G <: AbstractDFG
  return get2DSamples(fg, regexKey=regexKey, from=from, to=to )
end

function get2DPoseMeans(fg::G; from::Int=0, to::Int=(2^Sys.WORD_SIZE-1), regexKey=r"x") where G <: AbstractDFG
...
function get2DPoseMax(fgl::G;
					  regexKey::Regex=r"x",
                      from::Int=-(2^Sys.WORD_SIZE-1), to::Int=(2^Sys.WORD_SIZE-1) ) where G <: AbstractDFG
  #
