...
@test isapprox(mvn.Σ.mat[1,1], 1, atol=0.3)
@test isapprox(mvn.Σ.mat[2,2], 1, atol=0.3)
@test isapprox(mvn.Σ.mat[1,2], 0.7, atol=0.4)

@test isapprox(mvp.Σ.mat[1,1], 1, atol=0.3)
@test isapprox(mvp.Σ.mat[2,2], 1, atol=0.3)
@test isapprox(mvp.Σ.mat[1,2], -0.7, atol=0.4)

# sanity check for symmetry
@test mvn.Σ.mat - mvn.Σ.mat' |> norm < 0.01

# test means in the right location
@test isapprox(mvn.μ[1], 5.4, atol=0.75)
@test isapprox(mvn.μ[2], -4.8, atol=0.75)

@test isapprox(mvp.μ[1], 5.4, atol=0.75)
