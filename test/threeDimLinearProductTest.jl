...
global tf = SE3([0.0;0.0;0.0], AngleAxis(pi/4,[0;0;1.0]))# Euler(pi/4,0.0,0.0) )
...
@testset "Testing PriorPose3 evaluation..." begin
  ...
  global means = Base.mean(priorpts,2)
...
global X2ptsMean = Base.mean(X2pts,2)
global X1ptsMean = Base.mean(X1pts,2)
@show X1ptsMean
...
@testset "Ensure basic parameters on x1,x2 after inference..." begin
  ...
  global muX1 = Base.mean(getVal(fg,:x1),2)
  global stdX1 = Base.std(getVal(fg,:x1),2)
  ...
  global muX2 = Base.mean(getVal(fg,:x2),2)
  global stdX2 = Base.std(getVal(fg,:x2),2)
