...
function Base.convert(::Type{<:CameraCalibration}, dict::Dict)
    return CameraModels.CameraCalibration(;
        height = dict[:height],
        width = dict[:width],
...
function projectPointFrom(cam, c_H_w, w_Ph)
    ...
    return CameraModels.projectHomogeneous(cam, c_Ph)
end

function (cf::CalcFactor{<:GenericProjection{S, T}})(c_X, w_P_c, w_P_o) where {S, T}
    κ = 0.001
...
function solveMultiviewLandmark!(
    ...
    function cameraResidual(cam, meas, M, w_T_c, w_Ph, κ = 1000)
        ...
        # experimental cost function to try force bad reprojects in front of the camera during optimization
        return κ * (abs(pred.depth) - pred.depth)^2 +
               (meas[1] - pred[1])^2 +
               (meas[2] - pred[2])^2
    end
    ...
    _w_P_ci = Vector{
        ArrayPartition{Float64, Tuple{SVector{3, Float64}, SMatrix{3, 3, Float64, 9}}},
    }()
    vlbs = Symbol[]
