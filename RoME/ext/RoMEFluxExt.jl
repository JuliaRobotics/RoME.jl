...
function getSample(cfo::CalcFactor{<:MixtureFluxPose2Pose2})
    ...
    if nfb.DT[] == 0
        ...
        nfb.DT[] =
            (
                getTimestamp(cfo.fullvariables[2]) - getTimestamp(cfo.fullvariables[1])
            ).value * 1e-3
    end
...
function MixtureFluxPose2Pose2(
    fluxmodels::AbstractVector,
    inputDims::NTuple,
    data::AbstractArray,
    outputDims::NTuple,
    otherComponents::AbstractVector{<:SamplableBelief} = [
        MvNormal(zeros(3), diagm(ones(3)));
    ],
    diversity = [0.5; 0.5];
    DT::Real = 0,
    shuffle::Bool = true,
    ...
    #
    return MixtureFluxPose2Pose2(mix, Base.RefValue{Float64}(DT))
end
...
function Base.convert(
    ...
    #
    return PackedMixtureFluxPose2Pose2(convert(PackedMixture, obj.Z), obj.DT[])
end
...
function Base.convert(
    ...
    #
    return MixtureFluxPose2Pose2(convert(Mixture, obj.Z), Ref(obj.DT))
end

function calcVelocityInterPose2!(
    nfb::MixtureFluxPose2Pose2,
    ...
    if isnan(joyVelData[1, 3]) ||
       isinf(abs(joyVelData[1, 3])) ||
       isnan(joyVelData[1, 4]) ||
       isinf(abs(joyVelData[1, 4]))
        joyVelData[1, 3:4] .= 0.0
    end
    for i = 2:size(joyVelData, 1)
...
function (cfo::CalcFactor{<:MixtureFluxPose2Pose2})(meas1, meas2, Xi, Xj)
    ...
    cfZ = CalcFactorNormSq(
        cfo.factor.Z.mechanics,
        0,
        cfo._legacyParams,
        cfo.cache,
        cfo.fullvariables,
...
function FluxModelsPose2Pose2(
    ...
    DT::Real = 0.0,
    shuffle::Bool = true,
) where {P, M <: SamplableBelief, D <: AbstractMatrix}
    return MixtureFluxPose2Pose2(
        allModels,
        (25, 4),
        jvd,
        (3,),
