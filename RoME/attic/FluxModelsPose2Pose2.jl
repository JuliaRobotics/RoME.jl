...
struct FluxModelsPose2Pose2{P, D <: AbstractArray, M <: SamplableBelief} <:
...
function calcVelocityInterPose2!(
    ...
    if isnan(nfb.joyVelData[1, 3]) ||
       isinf(abs(nfb.joyVelData[1, 3])) ||
       isnan(nfb.joyVelData[1, 4]) ||
       isinf(abs(nfb.joyVelData[1, 4]))
        nfb.joyVelData[1, 3:4] .= 0.0
    end
    for i = 2:size(nfb.joyVelData, 1)
...
function convert(::Type{FluxModelsPose2Pose2}, d::PackedFluxModelsPose2Pose2)
    ...
    return FluxModelsPose2Pose2(
        MDS,
        joyVelM,
        extractdistribution(d.naiveModel),
        d.naiveFrac,
    )
end
...
function convert(::Type{PackedFluxModelsPose2Pose2}, d::FluxModelsPose2Pose2)
    ...
    for md in d.allPredModels
        ...
        mddict[:W1] = Vector{Float32}[vec(W1c[i]) for i = 1:length(W1c)]
        mddict[:b1] = md[1].b1
        mddict[:W2] = Vector{Float32}[vec(W2c[i]) for i = 1:length(W2c)]
        # mddict[:W2] = Vector{Vector{Float32}}(Vector{Float32}(W2c))
        mddict[:b2] = md[5].b
        mddict[:W3] = Vector{Float32}[vec(W3c[i]) for i = 1:length(W3c)]
        # mddict[:W3] = Vector{Vector{Float32}}(Vector{Float32}(W3c))
        mddict[:b3] = md[6].b
        push!(MDS, mddict)
    end
    joyVel = Vector{Float64}[d.joyVelData[i, :] for i = 1:size(d.joyVelData, 1)]
    return PackedFluxModelsPose2Pose2(joyVel, string(d.naiveModel), d.naiveFrac[], MDS)
end
