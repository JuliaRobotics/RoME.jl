...
for N in NN
    ...
    push!(
        alllikelihoods,
        DataFrame(;
            variable = union(["L$(i)" for i = 3:4], ["X$(i)" for i = 1:13]),
...
pl =
    plot(vDF, Geom.boxplot; x = :particles, y = :likelihood, Coord.cartesian(; xmax = 1025))
...
pl = plot(vDF, Geom.boxplot; x = :particles, y = :distance, Coord.cartesian(; xmax = 1025))

draw(PDF("slamedonutallPE.pdf", 12cm, 7cm), pl)

DFcomp = DataFrame(; particles = NN, seconds = computetime, time = "measured")
DFcompE = DataFrame(; particles = NN, seconds = computetime / 5, time = "possible")
