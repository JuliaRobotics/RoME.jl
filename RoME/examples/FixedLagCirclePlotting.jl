...
function plotCirc10BA(
    ...
    #
    plfl4 = drawPosesLandms(
        fg_;
        spscale = 1.5,
        manualColor = lineColor_,
        point_size = 4pt,
        drawhist = false,
        contour = false,
        levels = 2,
    ...
    for vsym in ellipseList_
        PL = plotCovEllipseLayer(
            fg_,
            vsym;
            points = vsym in pointsList_ || vsym in pointsList,
            ellipseColor = ellipseColor_,
        )
        if vsym != :x10
            for ll in PL
    ...
    plfl5 = drawPosesLandms(
        fg;
        spscale = 1.5,
        point_size = 3pt,
        manualColor = lineColor,
        drawhist = false,
        contour = false,
        line_width = 2pt,
    )
    ...
    for conl in contourList_
        ...
        plcon = plotKDEContour(
            marginal(X, [1; 2]);
            levels = levels,
            c = [contourColor_],
            line_width = contourLineWidth_,
        )
