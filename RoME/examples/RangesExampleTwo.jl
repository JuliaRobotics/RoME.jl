...
function addLandmsOnPose!(
    fgl::AbstractDFG,
    pose::Graphs.ExVertex,
    GTl::Dict{String, Float64};
    solvable = 0,
    N = 100,
)
    for gtl in GTl
...
function addNewPose!(fgl::AbstractDFG, from::Symbol, lbl::Symbol, GTp; solvable = 0, N = N)
    #
    init = 300 * randn(2, N)
    v = addVariable!(fgl, lbl, Point2; N = N, solvable = solvable)
...
@everywhere begin
    function drawQuadLandms(
        ...
        #
        h21 = if haskey(fgl.IDs, "l3")
            RoMEPlotting.drawMarginalContour(fgl, "l3")
        else
            Gadfly.context()
        end
        h22 = if haskey(fgl.IDs, "l4")
            RoMEPlotting.drawMarginalContour(fgl, "l4")
        else
            Gadfly.context()
        end
    ...
    function drawGroundTruth(
        ...
        for i = 2:(length(orderp))
            push!(
                LAYERS,
                Gadfly.layer(; x = [X[i - 1]; X[i]], y = [Y[i - 1]; Y[i]], Geom.path())[1],
            )
        end
        Xlr = Float64[]
        Ylr = Float64[]
        ...
        if length(orderl) > 0
            push!(
                ...
                Gadfly.layer(;
                    ...
                    y = Ylr,
                    Geom.point,
                    Theme(; default_color = colorant"red", point_size = 4pt),
                )[1],
            ) # label=Lblr, Geom.label
            push!(
                LAYERS,
                Gadfly.layer(;
                    x = Xla,
                    y = Yla,
                    Geom.point,
                    Theme(; default_color = colorant"cyan", point_size = 4pt),
                )[1],
            ) # label=Lbla, Geom.label
            if drawranges
...
function drawAllVidImages(GTp, GTl, fgl, ii, fr; drawranges = true, interp = false, t = 0)
    poselbls = ["l$(100+i)" for i = 0:ii]
    lmlbls = ["l$(i)" for i = 1:4]
...
function layerCircle(; cent = [0.0; 0.0], radius = 1.0, c = "deepskyblue", N = 200)
    ...
    return layer(; x = X, y = Y, Geom.path(), Theme(; default_color = parse(Colorant, c)))
end
function plotCircle(;
    cent = [0.0; 0.0],
    radius = 1.0,
    ...
    if !drawcent
        ...
        push!(
            ...
            layer(;
                ...
                y = [cent[2]],
                Geom.point,
                Theme(; default_color = parse(Colorant, c)),
            ),
        )
    end
...
function drawFirstPoseIllustration(GTl, GTp)
    ...
    PL = union(
        PL,
        plotCircle(; cent = [0.0; 0], radius = 3.0, c = "blue", drawcent = false).layers,
    )
    PL = union(
        PL,
        plotCircle(; cent = [36.0; 12], radius = 3.0, c = "blue", drawcent = false).layers,
    )

    PL =
        union(PL, plotCircle(; cent = l1, radius = rho1, c = "red", drawcent = true).layers)
    PL =
        union(PL, plotCircle(; cent = l2, radius = rho2, c = "red", drawcent = true).layers)
...
addLandmsOnPose!(fg, getVert(fg, :l101), landmsInRange(GTl, GTp["l101"]); N = N)
writeGraphPdf(fg)
batchSolve!(fg; N = N)
lk, pe = evaluateAccuracy(fg, GTp)
fid = open("/home/dehann/Videos/slamedonutLstPoseLks_$(N).txt", "a")
