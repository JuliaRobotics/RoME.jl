...
addFactor!(
    fg,
    [:l1;],
    PriorPoint2(MvNormal(GTl[:l1], Matrix{Float64}(LinearAlgebra.I, 2, 2))),
)
addFactor!(
    fg,
    [:l2;],
    PriorPoint2(MvNormal(GTl[:l2], Matrix{Float64}(LinearAlgebra.I, 2, 2))),
)
...
function vehicle_drives_to!(
    fgl::G,
    pos_sym::Symbol,
    GTp::Dict,
    GTl::Dict;
    measurelimit::R = 150.0,
) where {G <: AbstractDFG, R <: Real}
    currvar = ls(fgl)
    prev_sym = Symbol(
        "l$(maximum(Int[parse(Int,string(currvar[i])[2:end]) for i in 2:length(currvar)]))",
    )
...
pl = plotKDE(fg, [Symbol("l$(100+i)") for i = 0:2]; dims = [1; 2])
...
pl = plotKDE(fg, [Symbol("l$(100+i)") for i = 0:4]; dims = [1; 2]) |> PDF("/tmp/test.pdf")
...
pl = plotKDE(fg, [Symbol("l$(100+i)") for i = 2:8]; dims = [1; 2], levels = 6)
pl |> PDF("/tmp/test.pdf");
@async run(`evince /tmp/test.pdf`);
...
pl = plotKDE(fg, [Symbol("l$(100+i)") for i = 7:12]; dims = [1; 2])
pl |> PDF("/tmp/test.pdf");
@async run(`evince /tmp/test.pdf`);
