...
pl5blue = plotCirc10BA(
    fg5a,
    fg5a;
    filepath = joinLogPath(fg, "circ5A_blue.pdf"),
...
plotCirc10BA(
    fg_,
    fgI;
    filepath = joinLogPath(fg, "circ$(SIZE)IncrBA.pdf"),
    pointsList_ = [:x8],
)
...
pl10marg = plotCirc10BA(
    fg_,
    fgM;
    filepath = joinLogPath(fgM, "circ$(SIZE)MargBA.pdf"),
    pointsList = [:x5],
)
...
pl10recl = plotCirc10BA(
    fg_,
    fgR;
    filepath = joinLogPath(fg, "circ$(SIZE)ReclBA.pdf"),
    levels = 3,
    drawEllipse = true,
    ellipseList_ = [:x9],
)
...
include(
    joinpath(
        (Base.pathof(Caesar) |> splitpath)[1:(end - 2)]...,
        "examples",
        "marine",
        "auv",
        "Sandshark",
