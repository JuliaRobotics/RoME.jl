...
push!(lookup, [Symbol("l$i") for i = 1:10])
    ...
    for l = 1:LANDMARKS
        ...
        for (psid, ppbr) in BRdistr[lmidBr]
            ...
            addFactor!(
                fg,
                [opsid; lmid; lmid_0],
                Pose2Point2BearingRange(ppbr...);
                multihypo = [1; mainFrac; altFrac],
            )
        end
