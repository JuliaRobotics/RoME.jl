...
mutable struct PreintegralCompensationGradients <: PreintContainer
    ...
    # second order
    # ...
    function PreintegralCompensationGradients(;
        ...
        dRdDw::Array{Float64, 2} = zeros(3, 3),
    )
        return new(dPdDa, dVdDa, dPdDw, dVdDw, dRdDw)
    end
end
"""
$(TYPEDEF)
...
mutable struct InertialPose3Container <: PreintContainer
    ...
    pBa::Vector{Float64}
    rnTime::Int
    function InertialPose3Container(;
        ...
        return new(rRp, rPosp, rVelp, pBw, pBa, rnTime)
    end
end

function veeQuaternion(ip3::InertialPose3Container)
    ret = zeros(16)
    return ret[1:7] = veeQuaternion(SE3(ip3.rPosp, SO3(rRp)))
end
...
function zetaEmbedding(
    posei::InertialPose3Container,
    posej::InertialPose3Container;
    rGrav = [0.0; 0.0; 9.81],
)
    z = zeros(30)
...
function constructC1(
    posei::InertialPose3Container,
    pido::T,
    Dt::Float64,
) where {T <: PreintContainer}
    biRw = posei.rRp'
    C1 = zeros(15, 30)
    g1 = -biRw * Dt
    g2 = 0.5 * g1 * Dt
...
mutable struct InertialPose3 <: AbstractRelativeMinimize
    ...
    picg::PreintegralCompensationGradients
    reuse::Vector{
        Tuple{InertialPose3Container, InertialPose3Container, InertialPose3Container},
    } # number of threads

    function InertialPose3(
...
function convert(::Type{<:InertialPose3}, pip3::PackedInertialPose3)
    ...
    return InertialPose3(convert(SamplableBelief, pip3.Z), pioc, picg)
end
...
function PackedPriorInertialPose3(prip3::PriorInertialPose3)
    return PackedPriorInertialPose3(prip3.Zi.μ, prip3.Zi.Σ.mat[:], size(prip3.Zi.Σ.mat, 1))
end

function convert(::Type{PackedPriorInertialPose3}, prip3::PriorInertialPose3)
    return PackedPriorInertialPose3(prip3)
end

function convert(::Type{PriorInertialPose3}, pprip3::PackedPriorInertialPose3)
    return PriorInertialPose3(
        Distributions.MvNormal(pprip3.vecZi, reshapeVec2Mat(pprip3.vecCov, pprip3.dimc)),
    )
end
