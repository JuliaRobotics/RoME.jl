...
mutable struct reuseLBRA
    ...
    rbe::Vector{Float64}
    reuseLBRA() = new()
    function reuseLBRA(::Int)
        return new(
            SE3(0),
            Matrix{Float64}(LinearAlgebra.I, 4, 4),
            Euler(0),
            ones(4),
            ones(4),
            zeros(3),
...
mutable struct LinearRangeBearingElevation <: IIF.AbstractManifoldMinimize
    ...
    function LinearRangeBearingElevation(
        r::Tuple{Float64, Float64},
        b::Tuple{Float64, Float64};
        elev = Uniform(-0.25133, 0.25133),
    )
        return new(
            Normal(r...),
            Normal(b...),
            elev,
            reuseLBRA[reuseLBRA(0) for i = 1:Threads.nthreads()],
        )
    end
end
...
function ominus(
    ::Type{LinearRangeBearingElevation},
...
function ominus!(reuse::reuseLBRA, X::AbstractVector{<:Real}, L::AbstractVector{<:Real})
