...
function DynPose2VelocityPrior(
    z1::T1,
    z2::T2,
) where {T1 <: IIF.SamplableBelief, T2 <: IIF.SamplableBelief}
    return DynPose2VelocityPrior{T1, T2}(z1, z2)
end
...
function (cf::CalcFactor{<:DynPose2Pose2})(meas, wXi, wXj)
    #
...
function compare(a::DynPose2VelocityPrior, b::DynPose2VelocityPrior)::Bool
    return RoME.compareDensity(a.Zpose, b.Zpose) && RoME.compareDensity(a.Zvel, b.Zvel)
end
...
function (cf::CalcFactor{<:DynPose2DynPose2})(meas, wXi, wXj)
    #
    # FIXME update to Manifolds.jl usage
