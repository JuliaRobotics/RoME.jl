...
"""
mutable struct Pose2Point2BearingRange{
    B <: IIF.SamplableBelief,
    R <: IIF.SamplableBelief,
} <: IIF.AbstractManifoldMinimize
    bearing::B
    range::R
end

function DFG.getManifold(::IIF.InstanceType{<:Pose2Point2BearingRange})
    return SpecialOrthogonalGroup(2) × TranslationGroup(1)
end

function getSample(cfo::CalcFactor{<:Pose2Point2BearingRange})
    # defaults, TODO better reuse
    M = getManifold(cfo.factor)
    e0 = ArrayPartition([1 0; 0 1.0], [0.0])
...
function DFG.unpack(d::PackedPose2Point2BearingRange)
    return Pose2Point2BearingRange(
        convert(SamplableBelief, d.bearstr),
        convert(SamplableBelief, d.rangstr),
    )
end
