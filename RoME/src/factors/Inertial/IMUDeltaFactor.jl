...
Δ = [ΔR; Δv; Δp; Δt] 
"""
const SpecialGalileanGroup =
    LieGroup{ℝ, MatrixMultiplicationGroupOperation, SpecialGalileanManifold}#GroupManifold{ℝ, SpecialGalileanManifold, MultiplicationOperation}
function SpecialGalileanGroup()
    return LieGroup(SpecialGalileanManifold(), MatrixMultiplicationGroupOperation())
end
...
function affine_matrix(G::SpecialGalileanGroup, p::ArrayPartition{T}) where {T <: Real}
    return vcat(hcat(p.x[1], p.x[2], p.x[3]), @SMatrix [
        0 0 0 1 p.x[4]
        0 0 0 0 1
    ])
end

function vector_affine_matrix(
    G::SpecialGalileanGroup,
    X::ArrayPartition{T},
) where {T <: Real}
    return vcat(hcat(X.x[1], X.x[2], X.x[3]), @SMatrix [
        0 0 0 0 X.x[4]
        0 0 0 0 0
    ])
end
...
function LieGroups.inv(M::SpecialGalileanGroup, p)
    ...
    return ArrayPartition(ΔR', -ΔR' * Δv, -ΔR' * (Δp - Δv * Δt), -Δt)
end
...
function boxminus(::SpecialGalileanGroup, p, q; g⃗ = SA[0, 0, 9.81])
    ...
    return ArrayPartition(ΔR, Δv, Δp, Δt)
end
...
Base.@kwdef struct IMUDeltaFactor{T <: SamplableBelief} <: AbstractManifoldMinimize
    ...
    Δ::ArrayPartition{
        Float64,
        Tuple{SMatrix{3, 3, Float64, 9}, SVector{3, Float64}, SVector{3, Float64}, Float64},
    }
...
function IIF.getSample(cf::CalcFactor{<:IMUDeltaFactor})
    return exp(
        SpecialGalileanGroup(),
        hat(SpecialGalileanGroup(), SA[rand(cf.factor.Z)..., cf.factor.Δt]),
    )
end
...
function (cf::CalcFactor{<:IMUDeltaFactor})(
    ...
    _p::ArrayPartition{
        Float64,
        Tuple{MMatrix{3, 3, Float64, 9}, MVector{3, Float64}, MVector{3, Float64}},
    },
    _q::ArrayPartition{
        Float64,
        Tuple{MMatrix{3, 3, Float64, 9}, MVector{3, Float64}, MVector{3, Float64}},
...
function integrateIMUDelta(Δij, Σij, Δij_J_b, a, ω, a_b, ω_b, δt, Σy)
    ...
    ωδt = (ω - ω_b) * δt
    aδt = (a - a_b) * δt
    Xc = SVector{10, Float64}(vcat([0.0, 0, 0], aδt, ωδt, δt))
...
function PackedIMUDeltaFactor(; Z, dt, D, Sigma, J_b, b)
    ...
    _J_b = Float64.(J_b)
    _b = Float64.(b)
    return PackedIMUDeltaFactor(_Z, _dt, _D, _Sigma, _J_b, _b)
end
