...
function _addLandmarkBeehive!(
    ...
    isAlready, simPPE, genLabel = IIF._checkVariableByReference(
        fg,
        lastPose,
        landmarkRegex,
        RoME.Point2,
    ...
    for i = (posecount):(posecount + 5)
        ...
        if !addLandmarks
            ...
            _addLandmarkBeehive!(
                ...
                nsym;
                refKey = refKey,
                solvable = landmarkSolvable,
                atol = atol,
                graphinit = false,
                _doHoneycomb = _doHoneycomb,
            )
        end
    end
...
"""
function generateGraph_Honeycomb!(
    poseCountTarget::Int = 36;
    graphinit::Bool = false,
    dfg::AbstractDFG = LocalDFG{SolverParams}(;
        solverParams = SolverParams(; graphinit = graphinit),
    ),
    ...
    while posecount < poseCountTarget
        posecount = _driveHex!(
            ...
            posecount;
            graphinit = graphinit,
            landmarkSolvable = landmarkSolvable,
            atol = atol,
            poseCountTarget = poseCountTarget,
            postpose_cb = postpose_cb,
            _doHoneycomb = true,
        )
        ...
        if haskey(_honeycombRecipe, lastPose)
            posecount = _offsetHexLeg(
                dfg,
                posecount;
                direction = _honeycombRecipe[lastPose],
                graphinit = graphinit,
                atol = atol,
                landmarkSolvable = landmarkSolvable,
                poseCountTarget = poseCountTarget,
