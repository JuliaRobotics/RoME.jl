...
function generateGraph_ZeroPose(;
    ...
    if (graphinit isa Nothing)
        ...
        @error(
            "generateGraph_ZeroPose keyword graphinit obsolete, use solverParams(graphinit=..) instead."
        )
    end
    ...
    if !exists(dfg, label)
        ...
        _addPoseCanonical!(
            ...
            variableTags,
            factorTags,
            graphinit = solverParams.graphinit,
            doRef,
            postpose_cb,
        )
        #
...
function buildGraphChain!(
    ...
    doRef::Bool = true,
    dfg::AbstractDFG = generateGraph_ZeroPose(;
        varType,
        solverParams,
        solvable,
        doRef,
...
"""
function generateGraph_TwoPoseOdo(;
    solverParams::SolverParams = SolverParams(),
    ...
    if (graphinit isa Nothing)
        ...
        @error(
            "generateGraph_TwoPoseOdo keyword graphinit obsolete, use solverParams=SolverParams(graphinit=..) instead."
        )
    end
    if (fg isa Nothing)
        nothing
    else
        @error("generateGraph_TwoPoseOdo keyword fg obsolete, use dfg= instead.")
    end

    buildGraphChain!([MvNormal([10.0; 0; 0.0], diagm([1.0; 1.0; 0.01]));]; dfg, varType)
    ...
    if addlandmark
        ...
        addFactor!(
            dfg,
            [:x1; :l1],
            Pose2Point2BearingRange(Normal(0.0, 0.01), Normal(20.0, 1.0));
            graphinit = solverParams.graphinit,
        )
    end
...
function generateField_InertialMeasurement(;
    ...
    gn = if norm(σ_ω) < 1e-14
        () -> [0, 0, 0]
    else
        () -> rand(MvNormal(diagm(ones(3) * σ_ω^2 * 1 / dt)))
    end
    an = if norm(σ_a) < 1e-14
        () -> [0, 0, 0]
    else
        () -> rand(MvNormal(diagm(ones(3) * σ_a^2 * 1 / dt)))
    end
...
function generateField_InertialMeasurement_RateZ(;
    ...
    return generateField_InertialMeasurement(; dt, N, rate, kw...)
end

function generateField_InertialMeasurement_noise(;
    dt = 0.1,
    N = 11,
    rate = [0, 0, 0.001],
    gravity = SA[0, 0, 9.81],
    ...
    return generateField_InertialMeasurement(; dt, N, rate, gravity, accel0, σ_a, σ_ω)
end
