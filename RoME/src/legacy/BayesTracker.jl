...
function upplocal()
    ...
    locpid = (
        if N > 1
            ((locpid < N && locpid != 0 && N > 2) ? (locpid + 1) % (N + 1) : 2)
        else
            (N == 1 ? 1 : 2)
        end
    )
...
function propAllTrackers!(
    trkrs::Dict{Int, Feature},
    bDxb1::Array{Float64, 1},
    s::Array{Float64, 1},
)
...
function initTrackersFrom(bearan::Array{Float64, 2})
    ...
    tr = Dict{Int, Feature}()
    featid = 0
    for i = 1:size(bearan, 2)
        addNewFeatTrk!(tr, bearan[:, i], [0.5; 0.03])
    end
    return tr
end

function insertLkhdEvals!(
    lkhds::Array{Float64, 2},
    currFeats::Dict{Int, Feature},
    sightCart::Array{Float64, 2},
)
...
function addNewFeats!(
    trckr::Dict{Int, Feature},
    lkhds::Array{Float64, 2},
    lkpidx::Array{Int, 1},
    allmeas::Array{Float64, 2},
    nidx::Array{Int, 1},
)
    newfeIds = Int[]
...
function hardMatches!(
    ha::Dict{Int, Array{Float64, 1}},
    dmdm::Array{Float64, 2},
    lkpidx::Array{Int, 1},
    allmeas::Array{Float64, 2},
)
...
function measUpdateTrackers!(
    trkrs::Dict{Int, Feature},
    assoc::Dict{Int, Array{Float64, 1}},
    s::Array{Float64, 1},
)
