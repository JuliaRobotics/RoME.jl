...
mutable struct SLAMWrapper <: AbstractSLAM
    ...
    SLAMWrapper() = new()
    function SLAMWrapper(a, b, c)
        return new(a, b, c, :x1, SE3(0), Dict{Symbol, Int}(), Dict{Int, Symbol}())
    end # TODO to be deprecated
end
...
function SLAMWrapperLocal(;
    dfg::G = initfg(),
    poseCount::Int = 0,
    frameCounter::Int = 0,
    poseStride::Int = 10,
    helpers::SLAMCommonHelper = SLAMCommonHelper(),
    solveSettings::ManageSolveSettings = ManageSolveSettings(),
) where {G <: AbstractDFG}
    return SLAMWrapperLocal{G}(
        dfg,
        poseCount,
        frameCounter,
        poseStride,
...
"""
triggerSolve!(slam::SLAMWrapperLocal) =
    put!(slam.solveSettings.poseSolveToken, slam.poseCount)

function checkSolveStride(slam::SLAMWrapperLocal)
    return slam.poseCount % slam.solveSettings.solveStride == 0
end
...
function stopManageSolveTree!(slam::SLAMWrapperLocal)
    return slam.solveSettings.loopSolver = false
end
    ...
    ST = @async begin
        ...
        while mss.loopSolver
            ...
            if 0 < length(mss.poseSolveToken.data)
                ...
                setSolvableOldPoses!(
                    dfg;
                    youngest = getSolverParams(dfg).qfl + round(Int, mss.solveStride / 2),
                    oldest = 100,
                    solvable = 0,
                )
                ...
                if !dbg
                    ...
                    saveDFG(dfg, joinpath(getLogPath(dfg), "fg_before_$(lasp)"))
                end
                ...
                if !dbg
                    ...
                    saveDFG(dfg, joinpath(getLogPath(dfg), "fg_after_$(lasp)"))
                end
                ...
                if 0 < length(lastList)
                    ...
                    mss.drtCurrent =
                        (lastSolved, Symbol("drt_" * string(lastSolved)[2:end]))
                end
            ...
            println(
                timinglog,
                "$solvecycle, $t0, $dt_wait, $dt_solvable, $dt_init, $dt_disengage, $dt_save1, $dt_solve, $(tree.buildTime), $dt_finish",
            )
        end
    end
    return ST
