...
function homography_to_coordinates(
    ...
    return [H[1:3, 4]; vee(LieAlgebra(G), log(G, H[1:3, 1:3]))]
end
...
function convert(::Type{_Rot.QuatRotation}, q::TransformUtils.Quaternion)
    return _Rot.QuatRotation(q.s, q.v...)
end
function convert(::Type{_Rot.QuatRotation}, x::SO3)
    q = convert(TransformUtils.Quaternion, x)
    return convert(_Rot.QuatRotation, q)
end
function convert(::Type{T}, x::SO3) where {T <: CoordinateTransformations.AffineMap}
    return LinearMap(convert(_Rot.QuatRotation, x))
end

function convert(::Type{T}, x::SE3) where {T <: CoordinateTransformations.AffineMap}
    return Translation(x.t...) ∘ convert(AffineMap{_Rot.QuatRotation{Float64}}, x.R)
end
function convert(
    ::Type{SE3},
    x::T,
) where {T <: CoordinateTransformations.AffineMap{_Rot.QuatRotation{Float64}}}
    return SE3(
        x.translation[1:3],
        TransformUtils.Quaternion(x.linear.w, [x.linear.x, x.linear.y, x.linear.z]),
    )
end
