...
function parseG2oInstruction!(
    ...
    elseif instruction[1] == "EDGE_SE2"
        ...
        info_mat = [
            parse(Float64, instruction[7]) parse(Float64, instruction[8]) parse(Float64, instruction[9])
...
function stringG2o!(
    ...
    INF[INF .== Inf] .= 0
    # get command
    comm = if !haskey(overwriteMapping, Pose2Pose2)
        commands[Pose2Pose2]
    else
        overwriteMapping[Pose2Pose2]
    end
...
function stringG2o!(
    ...
    # get command
    comm = if !haskey(overwriteMapping, Pose2Point2BearingRange)
        commands[Pose2Point2BearingRange]
    else
        overwriteMapping[Pose2Point2BearingRange]
    end
...
function stringG2o!(
    ...
    # get command
    comm = if !haskey(overwriteMapping, Pose3Pose3)
        commands[Pose3Pose3]
    else
        overwriteMapping[Pose3Pose3]
    end
...
function stringG2o!(
    ...
    #
    global commands
    return error("unknown factor type $fnc")
end
...
function _doG2oLoop(io, dfg, label, i, solveKey)
    ...
    return fnc(io, dfg, label, i, solveKey)
end

function _writeG2oVertexes(io, dfg::AbstractDFG, varIntLabel::OrderedDict, solveKey::Symbol)
    #
...
function _writeG2oFactors(
    ...
    for vs in vars
        ...
        for fc in kvfcs
            # ignore priors
            if ignorePriors && isPrior(dfg, fc)
                (filter!(x -> x != fc, fcts); continue)
            else
                nothing
            end
