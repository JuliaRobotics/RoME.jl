...
function setSolvableOldPoses!(
    ...
    oldPoses =
        setdiff(getLastPoses(dfg; filterLabel = filterLabel, number = oldest), newPoses)
...
function listVariablesLabelsWithinRange(
    ...
    for id in saids
        ...
        if occursin(regexKey, string(id)) &&
           (from != 0 || to != (2^(Sys.WORD_SIZE - 1) - 1))
            vertlbl = string(id)
...
function get2DSampleMeans(
    ...
    vsyms =
        listVariablesLabelsWithinRange(fg, regexKey; from = from, to = to, minnei = minnei)
...
function get2DLandmMeans(
    fg::G;
    from::Int = 0,
    to::Int = (2^(Sys.WORD_SIZE - 1) - 1),
    minnei::Int = 0,
    landmarkRegex::Regex = r"l",
) where {G <: AbstractDFG}
    #
    return get2DSampleMeans(fg, landmarkRegex; from = from, to = to, minnei = minnei)
end

function removeKeysFromArr(
    fgl::G,
    torm::Array{Int, 1},
    lbl::Array{String, 1},
) where {G <: AbstractDFG}
...
function removeKeysFromArr(fgl::G, torm::Array{Int, 1}, lbl::Array{Symbol, 1}) where {G}
    #
    return removeKeysFromArr(fgl, torm, string.(lbl))
end

function get2DLandmMax(
    fgl::G;
    from::Int = -(2^(Sys.WORD_SIZE - 1) - 1),
    to::Int = (2^(Sys.WORD_SIZE - 1) - 1),
    showmm = false,
...
function addLinearArrayConstraint(
    fgl::G,
    ...
    cl = LinearRangeBearingElevation(
        (rangebearing[1], rangecov),
        (rangebearing[2], bearingcov),
    )
...
function addSoftEqualityPoint2D(
    fgl::G,
    l1::Symbol,
    l2::Symbol;
    ...
    return addFactor!(fgl, [l1, l2], pp; solvable = solvable)
end
