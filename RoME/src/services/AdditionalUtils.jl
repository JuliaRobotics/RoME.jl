...
"""
function warmUpSolverJIT(;
    fg::AbstractDFG = generateGraph_Hexagonal(),
    drawtree::Bool = false,
)
    #
...
function makePosePoseFromHomography(
    ...
    len = size(pHq, 1) - 1
    M = LeftInvariantMetricSE(len)
    e0 = ArrayPartition(SVector(0, 0, 0.0), SMatrix{len, len}(1, 0, 0, 0, 1, 0, 0, 0, 1.0))
    ...
    if len == 2
        ...
        covar_ = [
            covar[1, 1] covar[1, 2] covar[1, end]
            covar[2, 1] covar[2, 2] covar[2, end]
            covar[end, 1] covar[end, 2] covar[end, end]
        ]
