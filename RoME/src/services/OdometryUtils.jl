...
"""
function accumulateDiscreteLocalFrame!(
    ...
    if 1e-4 < check
        @warn(
            "Covk1 is getting dangerously non-Hermitian, still forcing symmetric covariance matrix."
        )
    else
        nothing
    end
...
"""
function duplicateToStandardFactorVariable(
    ::Type{Pose2Pose2},
    mpp::MutablePose2Pose2Gaussian,
    dfg::AbstractDFG,
    prevsym::Symbol,
    ...
    solvable::Int = 1,
    graphinit::Bool = true,
    cov::Union{Nothing, Matrix{Float64}} = nothing,
)::Symbol
    #
...
function triggerPose(x, xprev, Tnow, Tprev, distrule, timerule, yawrule)
