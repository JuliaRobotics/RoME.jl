...
function addSimMapFG!(fg::AbstractDFG, lms::Array{Float64, 2})
    for i = 1:size(lms, 2)
        newLandm!(
            ...
            string('l', i),
            vectoarr2(lms[:, i]),
            0.001 * Matrix{Float64}(LinearAlgebra.I, 2, 2),
        )
    end
    return nothing
end
...
function simOdo!(
    ...
    fg::AbstractDFG,
    DX::Array{Float64, 1};
    noiserate = 2.0 * [3e-2; 3e-2; 1.5e-3],
    driftrate = [0.0; 0.0; 0.0],
    detLM = Union{},
)
...
```
"""
function calcPosePointBearingRange(pose::Vector{<:Real}, poin::Vector{<:Real})
    #
...
function showTruePredBR(
    ...
    return println(
        ps,
        lm,
        ": true BR=$(round(truA,digits=3)), pred BR=$(round(preA,digits=3)), mala=$(round(mala,digits=3))",
    )
end

function crossMalaBR(
    fgGT::AbstractDFG,
    fg::AbstractDFG,
    ps::String,
    lmT::String,
