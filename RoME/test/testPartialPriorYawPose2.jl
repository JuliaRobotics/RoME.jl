...
##
    ...
    addFactor!(fg, [:x], PartialPriorYawPose2(Normal(0, 0.01))) #MvNormal([0.,0, 0], [0.01, 0.01, 0.01])))
    addFactor!(fg, [:l], PriorPoint2(MvNormal([1.0, 1], [0.01, 0.01])))

    # tested with Yaw prior 0.5 and it worked, reverting to all up test
...
##
    ...
    addFactor!(fg, [:l], PriorPoint2(MvNormal([10.0, 10], [0.01, 0.01])))
