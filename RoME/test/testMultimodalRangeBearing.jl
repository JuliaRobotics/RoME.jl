...
function getSample(cfo::CalcFactor{<:NorthSouthPartial})
    return samplePoint(TranslationGroup(1), cfo.factor.Z)
end
...
@testset "test standard multimodal range bearing factor setup..." begin
    ...
    addFactor!(
        fg,
        [:l1],
        PriorPoint2(MvNormal([10.0; 0.0], Matrix(Diagonal([1.0; 1.0] .^ 2)))),
    )
    addFactor!(
        fg,
        [:l2],
        PriorPoint2(MvNormal([30.0; 0.0], Matrix(Diagonal([1.0; 1.0] .^ 2)))),
    )
