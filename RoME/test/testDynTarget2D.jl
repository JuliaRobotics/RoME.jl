...
plotKDE(fg, sort(ls(fg; whereLabel =  contains( r"[xl]"))); dims = [1; 2], title = "Positions")

plotKDE(fg, sort(ls(fg; whereLabel =  contains( r"l"))); dims = [3; 4], title = "velocities")
...
# """
# mutable struct PriorPose3ZRP{T1,T2} <: IncrementalInference.AbstractPriorObservation where {T1 <: SamplableBelief, T2 <: SamplableBelief}
#   z::T1
#   rp::T2
#   partial::Tuple
