...
@testset "Test (partial) priors on Pose3" begin
    ...
    mv3 = MvNormal(SA[0.0; 0.0; 0.0], SMatrix{3, 3}(1, 0, 0, 0, 1, 0, 0, 0, 1.0))
...
@testset "Testing basic PriorPose3ZRP" begin
    ...
    f0 = addFactor!(
        ...
        PriorPose3(
            MvNormal(
                [0.0, 5.0, 9.0, 0.1, 0.0, pi / 2],
                diagm([1, 1, 1, 0.1, 0.1, 0.1] .^ 2),
            ),
        ),
    )
    #                               z                      ϕ    θ  
...
f0 = addFactor!(
    fg,
    [:x1;],
    PriorPose3(MvNormal([0.0, 0.0, 10, 0, 0, 0], 1e-2 * diagm(ones(6)))),
)
...
sigth = 0.0281
mu1 = [0.0; 0.0; -10.0]
prpz = PriorPose3ZRP(Normal(mu1[3], sigth), MvNormal(mu1[1:2], [sigx 0.0; 0.0 sigy] .^ 2))
...
@testset "test PriorPose3ZRP evaluations" begin
    ...
    @test sum(
        abs.(Statistics.mean(pts; dims = 2)[newdims] - mu1[[3; 1; 2]]) .< [0.5; 0.1; 0.1],
    ) == 3
...
@testset "test residual function of Pose3Pose3XYYaw" begin
    ...
    # noisy measurements
    xi = ArrayPartition([0; 0; 0.0], collect(RotZ(0)))
    xjb = exp(M, ϵ, hat(M, ϵ, Xjb))
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xjb))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # more rotated
    xi = ArrayPartition([0; 0; 0.0], collect(RotZ(π / 2)))
    xj = ArrayPartition([-5; 20; 0.0], [-1 0 0; 0 -1 0; 0 0 1.0])
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # add z
    xi = ArrayPartition([0; 0; 100.0], collect(RotZ(π / 2)))
    xj = ArrayPartition([-5; 20; -100.0], [-1 0 0; 0 -1 0; 0 0 1.0])
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # add pitch without z
    xi = ArrayPartition([0; 0; 0.0], collect(RotY(π / 4)))
    xj = ArrayPartition([20; 5; 0.0], collect(RotZ(π / 2) * RotY(π / 4)))
    # xj = ArrayPartition([14.14213562;5;-14.14213562], [0 -0.707107 0.707107; 1 0 0; 0 0.707107 0.707107] )
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # add pitch with z
    xi = ArrayPartition([0; 0; 10.0], collect(RotY(π / 4)))
    xj = ArrayPartition([20; 5; -10.0], collect(RotZ(π / 2) * RotY(π / 4)))
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # add roll without z
    xi = ArrayPartition([0; 0; 0.0], collect(RotX(π / 4)))
    xj = ArrayPartition([20; 5; 0.0], collect(RotZ(π / 2) * RotX(π / 4)))
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # add roll with z
    xi = ArrayPartition([0; 0; 10.0], collect(RotX(π / 4)))
    xj = ArrayPartition([20; 5; -10.0], collect(RotZ(π / 2) * RotX(π / 4)))
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # add roll with x
    xi = ArrayPartition([10; 0; 0.0], collect(RotX(π / 4)))
    xj = ArrayPartition([10 + 20; 5; 0.0], collect(RotZ(π / 2) * RotX(π / 4)))
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # add pitch and roll without z
    xi = ArrayPartition([0; 0; 0.0], collect(RotY(π / 4) * RotX(π / 6)))
    xj = ArrayPartition([20; 5; 0.0], collect(RotZ(π / 2) * RotY(π / 4) * RotX(π / 6)))
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)

    # add pitch and roll with x and z
    xi = ArrayPartition([10; 0; 10.0], collect(RotY(π / 4) * RotX(π / 6)))
    xj = ArrayPartition(
        [10 + 20; 5; -10.0],
        collect(RotZ(π / 2) * RotY(π / 4) * RotX(π / 6)),
    )
    res = calcFactorResidualTemporary(xyy, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [0; 0; 0], atol = 0.15)
    ...
    xj = [20]
    res =
        calcFactorResidualTemporary(lr, (ContinuousScalar, ContinuousScalar), [], (xi, xj))
    ...
    xi = ArrayPartition([10; 0; 10.0], collect(RotZ(0.0)))
    xj = ArrayPartition([20; 10; -10.0], collect(RotZYX(0.0, -pi / 4, pi / 4)))
    res = calcFactorResidualTemporary(xyy2, (Pose3, Pose3), [], (xi, xj))
    @test isapprox(res, [10; -5; 0], atol = 0.15)

    mu = [20.0, 5.0, pi / 4]
    xyy2 = Pose3Pose3XYYaw(MvNormal(mu, diagm([0.01, 0.01, 0.001] .^ 2)))
    xi = ArrayPartition([10; 0; 10.0], collect(RotZ(pi / 2)))
    xj = ArrayPartition([20; 10; -10.0], collect(RotZYX(0.0, -pi / 4, pi / 4)))
    res = calcFactorResidualTemporary(xyy2, (Pose3, Pose3), [], (xi, xj))
...
@testset "test Pose3Pose3XYYaw evaluations" begin
    ...
    @test isapprox(
        submanifold_component(mean(_X2prd, false), 1),
        submanifold_component(mean(_X2, false), 1),
        atol = 2.0,
    )
    @test isapprox(
        submanifold_component(mean(_X2prd, false), 2),
        submanifold_component(mean(_X2, false), 2),
        atol = 0.5,
    )
    ...
    for i = 1:N
        @test isapprox(
            submanifold_component(_X2prd_[i], 1)[3],
            submanifold_component(_X2pts_[i], 1)[3],
            atol = 0.0001,
        )

        @test isapprox(
            convert(TU.Euler, SO3(collect(submanifold_component(_X2prd_[i], 2)))).R,
            convert(TU.Euler, SO3(collect(submanifold_component(_X2pts_[i], 2)))).R,
            atol = 0.2,
        )

        @test isapprox(
            convert(TU.Euler, SO3(collect(submanifold_component(_X2prd_[i], 2)))).P,
            convert(TU.Euler, SO3(collect(submanifold_component(_X2pts_[i], 2)))).P,
            atol = 0.2,
        )
    end
...
@testset "test predictbelief with two functions" begin
    ...
    _val =
        IIF.getCoordinates.(
            Pose3,
            getPoints(propagateBelief(fg, :x2, ls(fg, :x2); N = N)[1]),
        )
...
@testset "Test Pose3Pose3XYYaw combinations:" begin
    ...
    for xyz_rpy in testsMeasurements_xyz_rpy
        @info xyz_rpy
...
@testset "Square PriorPose3ZRP and Pose3Pose3XYYaw combination solve" begin
    ...
    for j = 1:2
        ...
        for i = 1:N
            prpz = PriorPose3ZRP(
                Normal(i, 0.1),
                MvNormal(σRP * randn(2), diagm([0.01, 0.01] .^ 2)),
            )
...
@testset "Test Pose3Pose3Rotation factor:" begin
    ...
    testsMeasurements_rpy =
        [[0, 0, 0], [ϕ, 0, 0], [0, θ, 0], [0, 0, ψ], [-ϕ, 0, 0], [0, -θ, 0], [0, 0, -ψ]]

    @info "Test Pose3Pose3Rotation cases:"
    rpy = testsMeasurements_rpy[1]
    for rpy in testsMeasurements_rpy
        @info rpy
        wTx1 = getPointIdentity(Pose3)
        wTx2 = ArrayPartition([0.0, 0, 0], Matrix(RotXYZ(rpy...)))
