...
##
    ...
    @cast cs[j, i] := DistributedFactorGraphs.getCoordinates.(Pose2, getPoints(getBelief(fg, :x0)))[i][j]
    @test 35 < sum(-3.0 .< cs[1, :] .< 3.0)
    @test 35 < sum(-3.0 .< cs[2, :] .< 3.0)
    @test 35 < sum(-0.3 .< cs[3, :] .< 0.3)

    @cast cs[j, i] := DistributedFactorGraphs.getCoordinates.(Pose2, getPoints(getBelief(fg, :x1)))[i][j]
    @test 35 < sum(7.0 .< cs[1, :] .< 13.0)
    @test 35 < sum(-3.0 .< cs[2, :] .< 3.0)
    @test 35 < sum(0.7 .< cs[3, :] .< 1.3)

    @cast cs[j, i] := DistributedFactorGraphs.getCoordinates.(Pose2, getPoints(getBelief(fg, :x2)))[i][j]
    @test 35 < sum(12.0 .< cs[1, :] .< 18.0)
    @test 35 < sum(6.0 .< cs[2, :] .< 11.0)
    @test 35 < sum(1.8 .< cs[3, :] .< 2.4)

    @cast cs[j, i] := DistributedFactorGraphs.getCoordinates.(Pose2, getPoints(getBelief(fg, :x3)))[i][j]
    @test 35 < sum(7.0 .< cs[1, :] .< 13.0)
    ...
    @cast cs[j, i] := DistributedFactorGraphs.getCoordinates.(Pose2, getPoints(getBelief(fg, :x4)))[i][j]
    @test 35 < sum(-5.0 .< cs[1, :] .< 5.0)
    @test 35 < sum(13.0 .< cs[2, :] .< 22.0)
    @test 35 < sum(-2.8 .< cs[3, :] .< -1.5)

    @cast cs[j, i] := DistributedFactorGraphs.getCoordinates.(Pose2, getPoints(getBelief(fg, :x5)))[i][j]
    @test_broken 35 < sum(-8.0 .< cs[1, :] .< -2.0)
    @test_broken 35 < sum(6.0 .< cs[2, :] .< 11.0)
    @test 35 < sum(-1.3 .< cs[3, :] .< -0.7)

    @cast cs[j, i] := DistributedFactorGraphs.getCoordinates.(Pose2, getPoints(getBelief(fg, :x6)))[i][j]
    @test_broken 35 < sum(-3.0 .< cs[1, :] .< 3.0)
    @test 35 < sum(-3.0 .< cs[2, :] .< 3.0)
    @test_broken 35 < sum(-0.3 .< cs[3, :] .< 0.3)

    @cast cs[j, i] := DistributedFactorGraphs.getCoordinates.(Point2, getPoints(getBelief(fg, :l1)))[i][j]
