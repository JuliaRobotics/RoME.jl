...
@testset "ensure solveParametricBinary is working" begin
    ...
    q = ϵ  # variable to
    @test isapprox(
        abs.(calcFactorResidualTemporary(pp2, (Pose2, Pose2), X, (p, q))),
        [0; 0; pi],
    )

    q = exp(M, ϵ, hat(M, ϵ, [0; 0; -pi])) # variable to
    @test isapprox(
        calcFactorResidualTemporary(pp2, (Pose2, Pose2), X, (p, q)),
        [0; 0; 0],
        atol = 1e-14,
    )
...
@testset "Test canonical FG Honeycomb generation" begin
    ...
    t, m, gn = IIF._checkVariableByReference(
        fg,
        :x2,
        r"x\d+",
        Pose2,
