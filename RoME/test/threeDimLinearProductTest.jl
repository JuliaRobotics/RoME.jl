...
@testset "test 3D convolutions and products" begin
    ...
    [initCov[i, i] = 0.000001 for i = 4:6]
    ...
    X =
        IIF.getPoint.(
            Pose3,
            [[0.01 * randn(5); (0 * pi / 4 .+ 0.01 * randn(1))] for _ = 1:N],
        )
...
@testset "Adding PriorPose3 to graph..." begin
    v1 = addVariable!(fg, :x1, Pose3; N = N) # 0.1*randn(6,N)
...
@testset "Adding Pose3Pose3 to graph..." begin
    ...
    points = [deepcopy(ϵ), exp(G, hat(G, ϵ, [10.0; 0; 0; normalize([1, 1, 1]) * 2pi / 3]))]

    meas_coords = [
        SA[10.0, 0, 0, 0, 0, 0],
        SA[10.0, 0, 0, 0, 0, pi - 0.01],
        SVector{6}([10.0; 0; 0; normalize([1, 1, 1]) * (pi - 1e-3)]),
    ]
    ...
    @cast pts[j, i] := IIF.getCoordinates.(Pose3, _pts)[i][j]
...
@testset "Ensure basic parameters on x1,x2 after inference..." begin
    ...
    stdX1 = Statistics.std(pts[:, mask]; dims = 2)
    ...
    stdX2 = Statistics.std(pts[:, mask]; dims = 2)
