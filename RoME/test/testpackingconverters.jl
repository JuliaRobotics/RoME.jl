...
if false
    @testset "test PriorPoint2" begin
        ...
        global prpt2 =
            PriorPoint2(MvNormal([0.25; 0.75], Matrix(Diagonal([1.0; 2.0] .^ 2))))
    ...
    @testset "test conversions of PriorPose2" begin
        ...
        unpackeddata = reconstFactorData(
            fg,
            getVariableOrder(f1),
            IIF.FunctionNodeData{IIF.CommonConvWrapper{RoME.PriorPose2}},
            packeddata,
        )
    ...
    @testset "test conversions of Pose2Pose2" begin
        ...
        global packeddata = convert(
            IncrementalInference.PackedFunctionNodeData{RoME.PackedPose2Pose2},
            DFG.getSolverData(f2),
        )
        global unpackeddata = reconstFactorData(
            fg,
            getVariableOrder(f2),
            IIF.FunctionNodeData{IIF.CommonConvWrapper{RoME.Pose2Pose2}},
            packeddata,
        )
    ...
    @testset "test conversions of Pose2Point2BearingRange" begin
        ...
        global ppbr = Pose2Point2BearingRange(Normal(0.0, 0.005), Normal(50, 0.5))
        ...
        global packeddata = convert(
            IncrementalInference.PackedFunctionNodeData{RoME.PackedPose2Point2BearingRange},
            DFG.getSolverData(f3),
        )
        global unpackeddata = reconstFactorData(
            fg,
            getVariableOrder(f3),
            IncrementalInference.FunctionNodeData{
                IIF.CommonConvWrapper{RoME.Pose2Point2BearingRange},
            },
            packeddata,
        ) # IncrementalInference.FunctionNodeData{IIF.CommonConvWrapper{RoME.Pose2Point2BearingRange{Normal{Float64},Normal{Float64}}}}
    ...
    @testset "test conversions of PriorPose3" begin
        ...
        global packeddata = convert(
            IncrementalInference.PackedFunctionNodeData{RoME.PackedPriorPose3},
            DFG.getSolverData(f1),
        )
        global unpackeddata = reconstFactorData(
            fg,
            getVariableOrder(f1),
            IncrementalInference.FunctionNodeData{IIF.CommonConvWrapper{RoME.PriorPose3}},
            packeddata,
        )
        ...
        @test compareAll(
            DFG.getSolverData(f1).fnc.usrfnc!,
            unpackeddata.fnc.usrfnc!,
            skip = [:Zi; :Z; :p],
        )
        ...
        @test compareAll(
            DFG.getSolverData(f1).fnc.usrfnc!.Z,
            unpackeddata.fnc.usrfnc!.Z,
            skip = [:Σ;],
        )
        @test compareAll(
            DFG.getSolverData(f1).fnc.usrfnc!.Z.Σ,
            unpackeddata.fnc.usrfnc!.Z.Σ,
        )
    ...
    @testset "test conversions of Pose3Pose3" begin
        ...
        global packeddata = convert(
            IncrementalInference.PackedFunctionNodeData{RoME.PackedPose3Pose3},
            DFG.getSolverData(f2),
        )
        global unpackeddata = reconstFactorData(
            fg,
            getVariableOrder(f2),
            IncrementalInference.FunctionNodeData{IIF.CommonConvWrapper{RoME.Pose3Pose3}},
            packeddata,
        )
    ...
    @testset "test conversions of PartialPriorRollPitchZ" begin
        global prpz =
            PriorPose3ZRP(MvNormal([0.0; 0.5], 0.1 * diagm([1.0; 1])), Normal(3.0, 0.5))
