...
@testset "test inflation on range solve" begin
    ...
    pts_ = [norm(pts_[i] - [-1000; 0]) for i = 1:N]
...
@testset "test bearing range with inflation, #380, IIF #1051" begin
    ...
    p2br = Pose2Point2BearingRange(
        Normal(rand(Normal(pi / 4, σ_bearing)), σ_bearing),
    ...
    p2br = Pose2Point2BearingRange(
        Normal(pi / 2 + rand(Normal(0, σ_bearing)), σ_bearing),
    ...
    p2br = Pose2Point2BearingRange(
        Normal(3pi / 4 + rand(Normal(0, σ_bearing)), σ_bearing),
