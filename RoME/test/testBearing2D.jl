...
@testset "Testing Bearing2D factor" begin
    ##
    M = RoME.LeftInvariantMetricSE(2)
    ϵ = getPointIdentity(M)
    ps = [exp(M, ϵ, hat(M, ϵ, [0.0, 0, 0]))]
    push!(ps, exp(M, ϵ, hat(M, ϵ, [5.0, 0, 0])))
    push!(ps, exp(M, ϵ, hat(M, ϵ, [10.0, 0, 0])))
    push!(ps, exp(M, ϵ, hat(M, ϵ, [10.0, 5, 0])))
    push!(ps, exp(M, ϵ, hat(M, ϵ, [10.0, 10, 0])))
    push!(ps, exp(M, ϵ, hat(M, ϵ, [5.0, 10, 0])))
    push!(ps, exp(M, ϵ, hat(M, ϵ, [0.0, 10, 0])))
    push!(ps, exp(M, ϵ, hat(M, ϵ, [0.0, 5, 0])))

    push!(ps, exp(M, ϵ, hat(M, ϵ, [0.0, 0, pi / 4])))
    push!(ps, exp(M, ϵ, hat(M, ϵ, [0.0, 0, -pi / 4])))

    push!(ps, exp(M, ϵ, hat(M, ϵ, [1.0, 2, 0]))) # [4,3,0]
    ...
    xi = ArrayPartition([0.0, 0], [1.0 0; 0 1])
    xj = [1.0, 1]
...
@testset "Simple Bearing2D test to give narrow vs broad belief posteriors" begin
    ...
    addFactor!(
        fg,
        [:x1],
        PriorPose2(MvNormal([10.0; 0; 0], diagm([0.01; 0.01; 0.001] .^ 2))),
    )
    addFactor!(
        fg,
        [:x2],
        PriorPose2(MvNormal([0.0; 10.0; 0], diagm([0.01; 0.01; 0.001] .^ 2))),
    )
    ...
    addFactor!(fg, [:l1], PriorPoint2(MvNormal([0.0, 0], diagm([10.0, 10] .^ 2))))
...
@testset "Triangulation test in 2D, 3 beacons" begin
    ...
    addFactor!(
        fg,
        [:l2],
        PriorPoint2(MvNormal([10.0 + sqrt(3) / 2; -0.5], lmp_noise));
        graphinit = false,
    )
    addFactor!(
        fg,
    ...
    addFactor!(
        fg,
        [:x1; :l3],
        Pose2Point2Bearing(Normal(-pi + pi / 6, 0.05));
        graphinit = false,
    )
...
@testset "Triangulation test in 2D (opposite), 3 beacons" begin
    ...
    addFactor!(
        fg,
        [:l1],
        PriorPoint2(MvNormal([-10.0; 1.0 - 10.0], lmp_noise));
        graphinit = false,
    )
    addFactor!(
        fg,
        [:l2],
        PriorPoint2(MvNormal([-10.0 + sqrt(3) / 2; -0.5 - 10.0], lmp_noise));
        graphinit = false,
    )
    addFactor!(
        fg,
